...
class BoostSH(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, base_estimator, n_iter=10, learning_rate=1.):
        """
    ...
    def predict_proba(self, X):
        ...
        predictions = np.zeros((len(index), len(self.classes)))
        for t in range(len(self.models)):
            if self.views_selected[t] in X.keys():
                ...
                predictions[int_index] += self.alphas[t] * self.models[t].predict_proba(X_test.values)
    ...
    def __init__(self, base_estimator, n_iter=10, learning_rate=1., sigma=0.15, gamma=0.3):
        """
            ...
            Arguments:
                ...
                learning_rate {float} -- Learning rate for boosting (default: 1)
        """
        super(IRBoostSH, self).__init__(base_estimator, n_iter, learning_rate)
        self.sigma = sigma
        self.gamma = gamma
